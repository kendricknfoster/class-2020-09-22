...
* What problem do we want to solve? Which courses have gained or lost people since September 1. 
* What data is relevant? September 1 and September 22 enrollment.
* What data is potentially missing? Workload in the first few weeks of class (how do we know? what are we measuring by? )
* How are we considering courses that dropped below 10 courses? 


After we have a model, Temperance suggests we should:

* Recognize the differences between pset classes and reading classes
* Recognize the effect of courses that dropped out of our dataset
...
sep_old <- sep_old %>%
  rename(total_old = total) 

sep_new <- sep_new %>%
  rename(total_new = total)

inner_join(sep_old, sep_new, by = c("id", "title", "name", "department")) %>%
  mutate(change = total_new - total_old) %>%
  arrange(desc(change)) %>%
  filter(change > 10 | change < -14) %>%
  ggplot(aes(fct_reorder(title, change), change, fill = department)) +
  geom_col() +
  theme_clean() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = "Biggest Enrollment Changes in Fall 2020", 
       x = "Course", 
       y = "Change in Enrollment", 
       caption = "Source: Harvard Office of the Registrar") +
  coord_flip
